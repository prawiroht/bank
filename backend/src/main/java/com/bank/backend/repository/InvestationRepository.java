...
public interface InvestationRepository extends JpaRepository<Investation, Long> {
    @Query(value = "SELECT i.INVESTATION_ID,b.BANK_ID,s.STATUS_ID,b.BANK_NAME,i.INVESTATION_NAME, " +
            "s.STATUS_NAME,us.USER_ID FROM INVESTATIONS i JOIN BANKS b " +
            "ON b.BANK_ID = i.BANK_ID JOIN STATUSES s ON s.STATUS_ID = i.STATUS_ID " +
            "JOIN USERS us ON us.USER_ID = i.USER_ID " +
            ...
            "OR LOWER(s.STATUS_NAME) LIKE LOWER(CONCAT(CONCAT('%',:pStatusName),'%'))", countQuery = "SELECT i.INVESTATION_ID,b.BANK_ID,s.STATUS_ID,b.BANK_NAME,i.INVESTATION_NAME, "
                    +
                    "s.STATUS_NAME,us.USER_ID FROM INVESTATIONS i JOIN BANKS b " +
                    "ON b.BANK_ID = i.BANK_ID JOIN STATUSES s ON s.STATUS_ID = i.STATUS_ID " +
                    "JOIN USERS us ON us.USER_ID = i.USER_ID " +
